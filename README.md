...
> 可重驗的實測對照（共用的 485 行任務 prompt ＋ 有/無 harness 兩個完整成品）：規格測得到的
> 維度打平（兩邊都 14/14 測試通過、零 console error），但規格寫不到的操控手感實測差距 5–8 倍
> （無 harness 版 50% 極速全舵即打轉；量測腳本與數據在 `benchmark/handling-metrics/`）——
> 為什麼會這樣、該怎麼讀，見 [`index.html`](index.html) 第六節。
